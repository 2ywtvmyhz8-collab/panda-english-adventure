...
const STORE='fox_academy_v201_fixed';
...
const cards=[
 ...
 {id:'beans',cat:'Discovery',emoji:'🫘',name:'Beans',type:'Protein',zh:'豆子',info:'Beans are protein food.'},
...
const grammar=[
 {q:'Whole fruit is ______ than fruit juice.',choices:['healthier','healthy','healthiest'],ans:'healthier',hint:'Use -er to compare two things.'},
 {q:'I ______ going to eat fruit.',choices:['am','is','are'],ans:'am',hint:'I am.'},
 {q:'She ______ going to exercise.',choices:['am','is','are'],ans:'is',hint:'She is.'},
 {q:'good 的比較級是？',choices:['better','gooder','best'],ans:'better',hint:'good is irregular: better.'}
];
const reading=[
 {q:'Who is Eva?',choices:['little sister','teacher','doctor'],ans:'little sister',hint:'Eva is the writer’s little sister.'},
 {q:'Which is healthier?',choices:['whole fruit','fruit juice','candy'],ans:'whole fruit',hint:'Whole fruit is healthier than fruit juice.'},
 {q:'Which grains are better?',choices:['whole grains','refined grains','no grains'],ans:'whole grains',hint:'Whole grains are better for you.'}
];
const bossPool=[
 {q:'Which food group is milk?',choices:['Dairy','Protein','Grains'],ans:'Dairy',hint:'Milk is dairy.',cardId:'milk'},
 {q:'Which food group is bread?',choices:['Grains','Fruits','Dairy'],ans:'Grains',hint:'Bread is made from grain.',cardId:'bread'},
 {q:'A group of wolves is a ____.',choices:['pack','pride','school'],ans:'pack',hint:'Wolves are a pack.',cardId:'wolf_pack'},
 {q:'A group of fish is a ____.',choices:['school','tower','swarm'],ans:'school',hint:'Fish swim in a school.',cardId:'fish_school'},
 {q:'Which is healthier?',choices:['whole fruit','fruit juice','fries'],ans:'whole fruit',hint:'Whole fruit is healthier.'},
 {q:'I ___ going to exercise.',choices:['am','is','are'],ans:'am',hint:'I am.'}
];

function save(){localStorage.setItem(STORE,JSON.stringify(S))}
function $(id){return document.getElementById(id)}
function go(id){document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));$(id).classList.add('active');window.scrollTo(0,0); if(id==='collection')renderCollection(); if(id==='review')renderReview(); if(id==='backpack')renderBackpack(); if(id==='homeFox')renderHome(); if(id==='boss')renderBoss(); update();}
...
function collect(id){if(!id)return; if(!S.cards[id]){S.cards[id]=1;toast('收藏：'+cardById(id).name)}}
...
function answer(btn,a,ans,k,label,hid,xp,cardId,quizObj){
 let n=Number(btn.parentElement.dataset.attempts||1);
 if(btn.disabled)return;
 if(a===ans){
   btn.classList.add('correct'); award(k,xp,n,cardId);
   btn.parentElement.querySelectorAll('button').forEach(b=>b.disabled=true);
 } else {
   btn.classList.add('wrong','locked'); btn.disabled=true;
   markWrong(k,label,quizObj); n++; btn.parentElement.dataset.attempts=n;
   if(n>=3&&hid)$(hid).classList.remove('hidden');
   toast('錯的選項已鎖住');
 }
}
...
function foodQuizzes(list){
 return list.map(c=>({q:`${c.emoji} ${c.name}`,choices:['Fruits','Vegetables','Grains','Protein','Dairy','Place','Nature','Nutrition','Animal Group'],ans:c.type,hint:c.info,cardId:c.id}));
}
...
function renderQuest(){let tasks=[['Vocabulary','完成核心單字','playWorld("Supermarket")'],['Reading','完成閱讀題','playReading()'],['Grammar','完成文法題','playGrammar()'],['Animal Note','老師補充','playWorld("Zoo")'],['Boss','打 Boss','go("boss")']];$('questList').innerHTML=tasks.map((t,i)=>`<div class="mission"><h3>${i+1}. ${t[0]}</h3><p>${t[1]}</p><button onclick='${t[2]}'>開始</button><button onclick="completeQuest(${i})">標記完成</button></div>`).join('')}
...
function playGrammar(){$('playCard').classList.remove('hidden');$('playTitle').textContent='Grammar School';$('playBox').innerHTML=shuffle(grammar).map((q,i)=>quizHtml(q,'grammar',i,6)).join('');go('world')}
function playReading(){$('playCard').classList.remove('hidden');$('playTitle').textContent='Library Reading';$('playBox').innerHTML=shuffle(reading).map((q,i)=>quizHtml(q,'reading',i,7)).join('');go('world')}
...
function renderReview(){
 let keys=Object.keys(S.review).filter(k=>!S.reviewed[k]);
 if(!keys.length){$('reviewBox').innerHTML='<div class="win">目前沒有待複習錯題。很棒！</div>';return}
 $('reviewBox').innerHTML=keys.map((k,i)=>{let r=S.review[k];return `<div class="reviewQuiz"><h3>${r.label}</h3><p>錯過 ${S.wrong[k]||0} 次｜待複習</p>${reviewQuestionHtml(k,r.quiz,i)}</div>`}).join('');
}
function reviewQuestionHtml(k,q,i){let hid='rev_h_'+i;return `<h3>${q.q}</h3><div class="hint hidden" id="${hid}">💡 Hint: ${q.hint}</div>${shuffle(q.choices).map(a=>`<button class="choice" onclick='answerReview(this,${JSON.stringify(a)},${JSON.stringify(q.ans)},${JSON.stringify(k)},${JSON.stringify(hid)},${JSON.stringify(q.cardId||null)})'>${a}</button>`).join('')}`}
function answerReview(btn,a,ans,k,hid,cardId){
 let n=Number(btn.parentElement.dataset.attempts||1);
 if(btn.disabled)return;
 if(a===ans){
   btn.classList.add('correct'); btn.parentElement.querySelectorAll('button').forEach(b=>b.disabled=true);
   S.reviewed[k]=1; S.xp+=10*star(n); S.coins+=2; if(cardId)collect(cardId);
   save();renderReview();update();toast('複習成功 +XP');
 } else {
   btn.classList.add('wrong','locked'); btn.disabled=true; n++; btn.parentElement.dataset.attempts=n;
   if(n>=3)$(hid).classList.remove('hidden'); toast('再想一次，錯誤選項已鎖住');
 }
}

function renderBoss(){
 $('bossWin').classList.add('hidden');
 $('bossHp').textContent='❤️❤️❤️❤️❤️';
 $('bossBox').innerHTML=shuffle(bossPool).slice(0,5).map((q,i)=>quizHtml(q,'boss',i,8,q.cardId)).join('') + `<button class="big" onclick="bossClear()">完成 Boss 戰</button>`;
}
function bossClear(){if(S.done['boss_'+today])toast('今天 Boss 已領過');else{S.done['boss_'+today]=1;S.badges.boss=1;S.coins+=120;S.xp+=60;$('bossWin').classList.remove('hidden');$('bossWin').innerHTML='🏆 Boss Defeated!<br>+120 Coins +60 XP<br>👾 Boss Badge';confetti();save();update();toast('Boss Clear!')}}
